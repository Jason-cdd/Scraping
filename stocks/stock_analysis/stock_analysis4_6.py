...
下跌6天，不同止盈标准m下收益的对比
"""
def get_data_analysis4_6(stock_code, stock_name, test_start, test_end):
    ...
    for i in range(test_start, test_end):
        if stock_nums[-1] == 0 and pctChgs[i-6] >= 0 and pctChgs[i] < 0 and pctChgs[i-1] < 0 and pctChgs[i-2] < 0 and pctChgs[i-3] < 0 and pctChgs[i-4] < 0 and pctChgs[i-5] < 0:                  # 账户空仓、当日下跌买入
            act = '买入'
    ...
    list_n6 = [round(account_profits_0[-1], 2),
    ...
    plt.title(stock_code + '(n=6, m=0~1.07)' + t1 + '--' + t2)
    ...
    plt.savefig('/home/cdd/Desktop/Scraping/stocks/stock_analysis/stock_data2/'
                + stock_code + stock_name + t1 + '--' + t2 + '(n=6)' + '.png')
    plt.show()
