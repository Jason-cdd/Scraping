...
class CoronaVirusSpider(object):
    ...
    def crawl_corona_virus_of_china_province(self):
        ...
        # print(last_day_corona_virus)
        # 定义列表，用于存储各国从1月23号至今的json数据
        corona_virus_data_of_china = []
        # 2.遍历各国疫情数据，获取统计的URL
        for country in tqdm(last_day_corona_virus_of_china, '采集1月22号以来各省疫情数据'):
            # 3.发送请求，获取各国从1月22号至今的json数据
            statistics_data_url = country['statisticsData']
            statistics_data_json_str = self.get_content_from_url(statistics_data_url)
            # print(statistics_data_json_str)
            # 4.把json数据转换为Python类型数据，添加列表中
            ...
            for one_day in statistics_data:
                one_day['provinceName'] = country['provinceName']
            # print(statistics_data)
            corona_virus_data_of_china.extend(statistics_data)
        # 5.把列表以json格式保存为文件
        self.save(corona_virus_data_of_china, 'data/corona_virus_data_of_china.json')
